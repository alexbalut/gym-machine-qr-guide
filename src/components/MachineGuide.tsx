  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-6">
      <div className="flex items-center justify-between gap-3 mb-5">
        ...
        <div className="flex rounded-xl border border-border overflow-hidden text-sm font-semibold">
          <button
            type="button"
            onClick={() => setLang("en")}
            className={`px-3 py-1.5 ${lang === "en" ? "bg-cyan-400 text-slate-950" : "bg-slate-900 text-slate-300"}`}
          >
            EN
          </button>
          <button
            type="button"
            onClick={() => setLang("fr")}
            className={`px-3 py-1.5 ${lang === "fr" ? "bg-cyan-400 text-slate-950" : "bg-slate-900 text-slate-300"}`}
          >
            FR
          </button>
        </div>
      ...
      <div className="mb-5 space-y-2">
        ...
        {workoutMsg && (
          <p className="text-sm text-center text-emerald-300" role="status">
            {workoutMsg}{" "}
            <Link href="/?tab=workout" className="underline underline-offset-2 hover:text-emerald-200">
              {copy.openWorkout}
            </Link>
          </p>
        )}
        ...
        <ol className="space-y-3">
          {data.steps.map((step, i) => (
            <li key={i} className="flex gap-3 text-slate-200">
              <span className="font-mono text-cyan-300 shrink-0 w-7">{String(i + 1).padStart(2, "0")}</span>
              <span>{step}</span>
            </li>
          ))}
        </ol>
